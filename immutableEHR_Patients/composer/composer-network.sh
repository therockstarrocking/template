...
function networkBuild () {
  ...
  echo $COMPOSER_NETWORK_NAME
    ...
    if composer card list -c PeerAdmin@${COMPOSER_NETWORK_NAME}-${DOMAIN} > /dev/null; then
        ...
        composer card delete -c alice@${COMPOSER_NETWORK_NAME}
        rm -rf ./cards/
        rm -rf ~/.composer/
    fi
...
function networkDeploy () {
    ...
    # composer network start -c PeerAdmin@samplenet-Patients -n samplenet -V 0.0.1 -A alice -C ./cards/alice/admin-pub.pem -A bob -C ./cards/bob/admin-pub.pem
...
function networkInstall () {
  ...
  if [ ! -d "alice" ]; then
    cd ../
    composer identity request -c PeerAdmin@${COMPOSER_NETWORK_NAME}-${DOMAIN} -u admin -s adminpw -d ./cards/alice
  fi
...
function networkStart () {
  ...
  composer network start -c PeerAdmin@${COMPOSER_NETWORK_NAME}-${DOMAIN} -n ${COMPOSER_NETWORK_NAME} -V ${NETWORK_ARCHIVE_VERSION} -A alice -C ./cards/alice/admin-pub.pem -A bob -C ./cards/bob/admin-pub.pem
...
function networAdminsCreate () {
  ...
  composer card create -p ./connection.json -u alice -n ${COMPOSER_NETWORK_NAME} -c alice/admin-pub.pem -k alice/admin-priv.pem -f ./cards/alice@${COMPOSER_NETWORK_NAME}.card
  composer card import -f ./cards/alice@${COMPOSER_NETWORK_NAME}.card
  composer network ping -c alice@${COMPOSER_NETWORK_NAME}
}
...
function networkUpgrade(){
  ...
  networkInstall
  composer network upgrade -c PeerAdmin@${COMPOSER_NETWORK_NAME}-${DOMAIN} -n ${COMPOSER_NETWORK_NAME} -V ${NETWORK_ARCHIVE_VERSION} -A alice -C ./cards/alice/admin-pub.pem -A bob -C ./cards/bob/admin-pub.pem
