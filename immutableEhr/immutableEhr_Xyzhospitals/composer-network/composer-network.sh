...
function Xip () {
  read -p "IP of ${DOMAIN2} :" XOIP
    if [ -z "XOIP" ]; then
        echo "Enter IP address of ${DOMAIN2}"
        Xip
    fi
}
function connectionCredentials () {
    ...
    IP=$(ifconfig | grep 192|awk 'NR==1{print $2}'| cut -c 6-)
    Xip
    echo $IP
    echo $XOIP
...
cat << EOF > ${DIR}/./connection-${CORG}.json
{
  ...
  "peers": {
    ...
    "peer1.${DOMAIN2}.example.com": {
      "url": "grpcs://${XOIP}:8051",
      "eventUrl": "grpcs://${XOIP}:8053",
      "grpcOptions": {
        "ssl-target-name-override": "peer1.${DOMAIN2}.example.com"
      },
      "tlsCACerts": {
        "pem": "${CA_CERT}"
      }
...
function buildComposer () {
    domain=$( echo "$DOMAIN2" | awk '{print tolower($0)}')
...
function networkInstall() {
 ...
 function networkBuild() {
   ...
   COMPOSER_NETWORK_NAME=immutableehr
    ...
    sed $OPTS "s/DOMAIN/${DOMAIN}/g" "${COMPOSE_FILE}"
    #sed $OPTS "s/DOMAIN2/patients/g" "${COMPOSE_FILE}"
