...
function verifyConfigFile(){
  if [ ! -f $NEW_ORG_NAME.json ]; then 
    ...
    scp -r ../channel-artifacts/crypto-config/ordererOrganizations/ ${SSH_ADDRESS}:./immutableEhr/immutableEhr_${NEW_ORG_NAME}/channel-artifacts/crypto-config/
    scp ${SSH_ADDRESS}:./immutableEhr/immutableEhr_${NEW_ORG_NAME}/channel-artifacts/${NEW_ORG_NAME}.json ../channel-artifacts/
    #exit 1
    if [ ! -f $NEW_ORG_NAME.json ]; then 
      verifyConfigFile
    fi
