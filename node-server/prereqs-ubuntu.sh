...
FABRIC_IMAGES_THIRDPARTY=(fabric-kafka fabric-zookeeper fabric-couchdb fabric-baseos)
for image in ${FABRIC_IMAGES[@]}; do
  echo "Pulling ${DOCKER_NS}/$image:${ARCH}-${VERSION}"
  docker pull ${DOCKER_NS}/$image:${ARCH}-${VERSION}
done

for imaget in ${FABRIC_IMAGES_THIRDPARTY[@]}; do
  echo "Pulling ${DOCKER_NS}/$imaget:${BASE_DOCKER_TAG}"
  docker pull ${DOCKER_NS}/$imaget:${BASE_DOCKER_TAG}
done
